...
The retained candidate evidence path uses witness-bound schema v2 and additive archive binding. The currently retained candidate binds the package, beta QA kit, packaged desktop witness companion and portable evidence-archive companion without claiming either remaining human gate. PR #93 additionally makes exact-main candidate retention explicit and testable through a fail-closed policy rather than requiring an artificial no-op commit. PR #96 adds a fail-closed currency guard for the packaged manual-QA tool; Candidate #133 is the fresh exact-main package-bound candidate selected after that guard was verified.
